...
function init() {
    fetch('bukshin.json') // сюда положи свой JSON
        ...
        .catch(err => {
            console.error('Ошибка загрузки VIP.json:', err);
        });
}
...
function extractQuantity(feature) {
    ...
    // 2) попытка вытащить из текста балуна
...
function setupFilterUI(minQuantity, maxQuantity, objectManager) {
    ...
    filterToggle.addEventListener('click', () => {
        const visibleNow = filterPanel.style.display === 'block';
        filterPanel.style.display = visibleNow ? 'none' : 'block';
    });
    ...
    if (categoryToggle && categoryPanel) {
        ...
        categoryToggle.addEventListener('click', () => {
            const visibleNow = categoryPanel.style.display === 'block';
            categoryPanel.style.display = visibleNow ? 'none' : 'block';
        });
    }
    ...
    // === ФИЛЬТР ПО ТИПАМ (Партнёр/Наша/Остальные) ===
    const updateTypeToggleState = () => {
        if (!typeToggle) return;
        if (showPartner && showNasha && showOther) {
    ...
    if (typeToggle && typePanel) {
        ...
        typeToggle.addEventListener('click', () => {
            const visibleNow = typePanel.style.display === 'block';
            typePanel.style.display = visibleNow ? 'none' : 'block';
        });
    }
...
function applyFilter(minQuantity, objectManager) {
    ...
    objectManager.setFilter(obj => {
        ...
        // --- ГИБДД: только флаг showGibdd, без остальных фильтров ---
        if (isBlue) {
            return showGibdd;
        }

        // --- остальные точки: фильтр по типам ---
        let markerType = props.markerType;
        if (!markerType) {
            // fallback по цвету, если markerType не записали
            if (preset === 'islands#greenIcon') {
                markerType = 'partner';
            } else if (preset === 'islands#yellowIcon') {
                markerType = 'nasha';
            } else {
                markerType = 'other';
            }
